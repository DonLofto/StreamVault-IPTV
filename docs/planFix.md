...
| **A27** Same URL parsed twice per channel | Carry `XtreamStreamToken` from `toEntity` forward so the URL is parsed once. **CORRECTED 2026-09-11: do NOT replace the reflective codec** — `XtreamUrlCodec` probes for the `(String, Charset)` overloads because they are API 33+, while `minSdk = 25`. On API 28 the probe returns null so no reflective invoke happens per call; calling the overload directly would compile but throw `NoSuchMethodError` on the target. Keep the probe. | `EntityMappers.kt:196`, `SyncCatalogStore.kt:822` | Assert 1 parse per channel | low (was med) | M |
