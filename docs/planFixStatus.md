...
**50 done · 3 partial · 1 superseded · 5 open.** Commits marked DONE are on `master`; the working
...
| **A17** | Partial. **RE-ANALYSED 2026-09-11 - the plan's premise is only half right.** The card says to push `matchesGuideMetadataSearch` into SQL, but that predicate cannot be the one that narrows the load: `scopedChannels` is not just the metadata-match set, it is the universe used to resolve `repositoryMatchedLookupKeys` (channels matched by a *program title*, returned by `searchPrograms`). Narrowing the load to metadata matches would silently drop every program-title hit whose channel name does not also match the query. **CONFIRMED, and one more constraint found (round 21).** The base snapshot cannot stand in for the scope load either: `baseSnapshot.allChannels` is capped at `MAX_CHANNELS` (60), because `loadPreferredGuideChannelsPage` returns `getChannelsByCategoryPage(providerId, ALL_CHANNELS_ID, MAX_CHANNELS)` - reusing it would silently restrict guide search to the first 60 channels. The scope load is deliberate. Caching it across keystrokes was considered and rejected too: the cached value is the provider's whole channel list, which would pin exactly the memory A58 is trying to release. The predicates that CAN be pushed safely are the two category-visibility filters at `EpgViewModel.kt:2077-2081` - hidden category ids and the accessible-category set - which are pure row filters and provably remove only rows the Kotlin code removes anyway. That needs a new DAO query plus a repository method, and a completeness test including non-ASCII channel names, because SQLite `LIKE` is case-insensitive for ASCII only while Kotlin `contains(ignoreCase = true)` folds Unicode. The guide-search passes (`buildSearchGuideSnapshot`: the lookup map, the metadata match, the matched-key build and the `mapNotNull`/`associateWith` over the result) now run in `withContext(Dispatchers.Default)` instead of `Dispatchers.Main.immediate`, so the UI thread is no longer doing several full passes over every channel of the provider on a path debounced at only 150 ms. **Still open:** `loadGuideSearchScopeChannels` still calls `channelRepository.getChannels(providerId).first()` with **no LIMIT**, so the whole provider channel table is still loaded into memory per search. Pushing the predicate into SQL is feasible — `matchesGuideMetadataSearch` matches `name` OR `categoryName`, both channel columns — but it needs a new DAO query plus the hidden-category and `accessibleCategoryIds` logic moved with it, and it risks changing search completeness. |
