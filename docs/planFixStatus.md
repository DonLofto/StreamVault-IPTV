...
**51 done · 2 partial · 1 superseded · 5 open.** Commits marked DONE are on `master`; the working
...
| **A17** | Implemented (`84833e08`), with one coverage gap. Both halves are done: the search passes already ran in `withContext(guideWorkDispatcher)`, and the two category-visibility filters now run in SQL via `ChannelRepository.getGuideSearchScopeChannels`, which reuses `observeChannels` so parental and hidden-channel visibility are unchanged. **Coverage gap:** the new SQL path has no Room-level test. It was verified against real SQLite for all three filter shapes (accessible+hidden, hidden only, accessible empty) and the `:app`/`:data` suites pass, but `EpgViewModelTest` selects a real category before searching, so it never reaches the `ALL_CHANNELS_ID` branch. **Two premises in the plan card were wrong** and are recorded so they are not re-tried: the metadata-match predicate cannot be pushed (it is not what narrows the load - see below), and the base snapshot cannot stand in for the load because `allChannels` is capped at `MAX_CHANNELS` (60). |
