...
| **A20** | Implemented (`7d2eeb27`): Stalker gets a dedicated `@StalkerClient` and the EPG client is genuinely isolated, both via `newIsolatedClient` (a plain `newBuilder()` shares Dispatcher and ConnectionPool by reference). Unit tests pin the defect, the fix and configuration inheritance at the helper level, and `NetworkModuleClientIsolationTest` (`25f7474d`) pins the DI wiring - the client the graph hands out for Stalker is isolated and bounded at 4/2 against the main client's 64/10, so a revert to a plain `newBuilder()` in the provider fails a test rather than silently re-sharing playback's slots. **Open:** the card rates this high risk because it changes network admission, so it needs the full live-TV protocol - blocked on the same memory-pressure freeze as A34. |
