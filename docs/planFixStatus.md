...
| ID | State |
|---|---|
| **A5** | Parse-time half DONE (`3b8cb2c5`: fallback `Regex` and `DateTimeFormatter` hoisted). **The exception-driven format probing itself is still open** — `parseDate` still constructs and throws up to 11 `DateTimeParseException`s per date, twice per programme. Needs `DateTimeFormatter.parse(CharSequence, ParsePosition)` or a single builder with `optionalOffset()`, guarded by golden-instant tests. |
