...
| **A25** | Repository half DONE (`68adb5a2`: `flowOn(Dispatchers.Default)` on movie/series `getCategories`). **Open:** the `app/ui` ViewModel `combine` transforms named in the audit (MoviesViewModel:145-171, SeriesViewModel:161/322, HomeViewModel:484, EpgViewModel:1101) ViewModel half PART DONE (`d3ece615`): the Movies and Series catalog-dependency combines now carry `.flowOn(Dispatchers.Default)`. **Still open:** HomeViewModel:461-513 and EpgViewModel:1098-1104. Both are only safe once they take the dispatcher from a binding rather than naming `Dispatchers.Default`, because their tests drive Main with a `StandardTestDispatcher` - attempting Home with a hard-coded `Default` failed `HomeViewModelTest` outright. The `DefaultDispatcher` binding added in the same commit is the mechanism to use. The EpgViewModel:1101 pointer in the audit does not correspond to heavy work in the current file (it builds a four-field data class), so it may be stale line drift. |
